...
const CategoryPage = () => {
    ...
    useEffect(() => {
        const fetchMovies = async () => {
            try {
                ...
                let data;
                switch (category) {
                    case 'films-tendances':
                        data = await fetchTrendingMovies(currentPage);
                        break;
                    case 'films-les-mieux-notes':
    ...
    const handlePageChange = (newPage) => {
        ...
        navigate(`/category/${category}?page=${newPage}`, { replace: true });
    ...
    const getCategoryTitle = () => {
        switch (category) {
            case 'films-tendances':
                return 'Films Tendances';
            case 'films-les-mieux-notes':
                return 'Films les Mieux Notés';
            case 'films-a-venir':
                return 'Films à Venir';
            case 'films-populaires':
        ...
        return (
            <div className="min-h-screen bg-[#091540] flex items-center justify-center">
                <div className="text-center">
                    <p className="text-red-500 mb-4">Une erreur s'est produite lors du chargement des films.</p>
        ...
        <div className="min-h-screen bg-[#091540] py-8">
            <div className="container mx-auto px-4">
                <div className="flex items-center justify-between mb-8">
                    <h1 className="text-3xl font-bold text-[#ABD2FA]">{getCategoryTitle()}</h1>
                    <Link to="/" className="text-[#7692FF] hover:text-[#ABD2FA] transition-colors">
                        Retour à l'accueil
                    </Link>
                </div>
